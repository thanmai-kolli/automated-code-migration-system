...
class JavaGenerator:
    ...
    def _generate_function(self, func, indent=1):
        ...
        for p in func.params:
            inferred_type = "int"  # default assumption
    ...
    def _generate_statement(self, stmt, indent):
        ...
        if isinstance(stmt, Variable):
            ...
            if (
                ...
                java_type = inferred_type
            else:
                java_type = self.mapper.map(
                    inferred_type,
                    self.source_lang,
                    self.target_lang
    )
        ...
        if isinstance(stmt, ForLoop):
            ...
            code = f"{tab}for (var {stmt.iterator} : {iterable_expr}) {{\n"
        ...
        if isinstance(stmt, ListAppend):
            return f"{tab}{self._generate_expr(stmt.list_obj)}.add({self._generate_expr(stmt.value)});\n"
    ...
    def _infer_type(self, value):
        ...
        if isinstance(value, ArrayLiteral):
            ...
            wrapped_type = self._to_wrapper(first_type)

            return f"List<{wrapped_type}>"
